 ...
 */
   ...
   /**
     ...
     * @return void
     */
		function set($pagesize=20,$total,$current=false, $jump_num = 5) {
			...
			if ($this->tpage > 1) {
				...
				if ($current>$jump_num) {
					$this->output_pre10 = '<a href='.$this->file.'&'.$this->pvar.'='.($current-$jump_num).($this->varstr).' title="前'. $jump_num .'页">&lt;&lt;&lt;</a>';
				}
				if ($current>1) {
					$this->output_pre = '<span class="link"><a href='.$this->file.'&'.$this->pvar.'='.($current-1).($this->varstr).' >上一页</a></span>';
				}

				$start	= floor($current/$jump_num)*$jump_num;
				$end	= $start+$jump_num-1;
				...
				if ($this->tpage>$jump_num && ($this->tpage-$current)>=$jump_num ) {
					$this->output_back10 = '<a href='.$this->file.'&'.$this->pvar.'='.($current+$jump_num).($this->varstr).' title="下' . $jump_num . '页">&gt;&gt;&gt;</a>';
				}
			}
