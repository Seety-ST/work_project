...
class pai_paizhao_consult_class extends POCO_TDG
{
    ...
    private $shot_url_prefix = 'http://t.yueus.com/';
    private $page_url_config;
    private $short_url_class;
    public function __construct()
    {
        include_once '/disk/data/htdocs232/poco/short_link/include/class.short_link.inc.php';
        $this->short_url_class = new short_link_class();
    ...
    {
		if($_COOKIE[$data['consult_cookie']])
		{
			$this->op_cookie('del', $data['consult_cookie']);
		}
		else
		{
			return array('result'=>-1, 'message'=>'非法操作');
		}
        ...
        if (!$data['consult_cookie'] || $this->find($where))
        {
            $return = array('result'=>-2, 'message'=>'您已留下过咨询信息啦!<br/>摄影顾问很快就会给您打电话啦!');
            $this->op_cookie('del', $data['consult_cookie']);
            return $return;
        }        
        $time = time();
...
//             return $rsp_arr['url'];
//         }
        $short_url = $this->short_url_class->add_short_link($long_link, $login_id);
        return $short_url ? $this->shot_url_prefix . $short_url : $long_link;
    }
