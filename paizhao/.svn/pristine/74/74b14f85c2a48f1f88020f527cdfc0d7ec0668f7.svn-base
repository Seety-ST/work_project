...
var yueyue = {
	...
	app_url: 'http://www.yueus.com',
	resource: 'http://static-c.yueus.com/',
	build: '',
...
function intval(mixed_var,base)
{
	...
	{
		tmp=mixed_var.replace(/[^0-9]/g, '');return(isNaN(tmp)||!isFinite(tmp))?0:tmp;
...
yueyue.setcookie = function( name, value, expires ) {
	setcookie(name, value, expires, '/', '.yueyue.com');
};

yueyue.data = function(k, v){
	if(v !== undefined){
		v = $.toJSON(v);
	}
...
yueyue.include = function(f, success){
	yueyue.include.including[f] = 1;
	...
	script.onload = script.onreadystatechange = function() {
		if ( !done && (!this.readyState ||
			...
			yueyue.include.included[f] = 1;
			delete yueyue.include.including[f];
...
yueyue.include_once = function(f, success){
	if(yueyue.include.including[f]){
		setTimeout(function(){ yueyue.include_once(f, success); }, 300);
		return false;
	}
	if(yueyue.include.included[f]){
		success(f);
		return true;
	}
	
	yueyue.include(f, success);
};

yueyue.include.included = {};
yueyue.include.including = {};
...
function P8_Dialog(options){
	...
	this.className = options.className || 'dialog_yueyue';
	...
	this.iframe = yueyue.ie6 && !this.overlay ? true : false;
...
function P8_CKEDITOR(options){
	...
	if(options.action == 'destroy'){
		...
		include_once(options.resource +'/ckeditor/ckeditor'+ (options.type ? '_'+ options.type : '') +'.js?'+ yueyue.build, function(){
...
$(function($){
	...
	if($_COOKIE['admin_url']){
		...
		if(count){
			...
			$('<a id="w3c_v" href="http://validator.w3.org/check?uri='+ encodeURIComponent(window.location.href) +'&group=1" style="cursor: pointer; position: fixed; top: 0px; left: 0px; z-index: 2147483647;" target="_blank" title="w3c validate"><img src="'+ yueyue.resource +'/images/w3c.png" width="36" height="24" /></a>').appendTo(document.body);
		}
		}catch(e){}
	}
	...
	$(document).keyup(function(e){
		if(e.altKey && e.keyCode == 57){
			if($_COOKIE['tp_test']){
				yueyue.setcookie('tp_test', 0, -1);
			}else{
				yueyue.setcookie('tp_test', 1);
			}
