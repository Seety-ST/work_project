...
foreach($ev_array as $key => $mix_value)
{
    if(!empty($mix_value))
    {
        ...
        {
            //参考京东，最多显示四个值，最后一个值不加间隔号
            if($k==($child_value_arr_len-1))
            {
                $split_str = "";
            }
            //匹配主分类对应的配置数组，匹配对应的筛选值
            $tmp_select_condition_arr[$j]["link_title"] .= $parent_list_to_config_arr[$parent_value][$v]["name"].$split_str;
        }
        $tmp_select_condition_arr[$j]["link_title"] = $title_str.$tmp_select_condition_arr[$j]["link_title"];
        //如果长度大于4条记录，则加点点

        //echo $child_value_arr_len;
        if($child_value_arr_len>4)
        {
            $more_str = "...";
            $tmp_select_condition_arr[$j]["name"] = $tmp_select_condition_arr[$j]["name"].$more_str;
        }
...
$tpl->assign("can_use_district",$can_use_district);
