...
fis.config.set('project.exclude', ['**/_*.scss','/**.cmd','tpl/**.tpl.html','yuefe-conf.js','iconfont_js/**']);
...
fis.config.get('roadmap.path').unshift({
    reg: /\/style\/ui\/(paizhao-ui-*.*)|(reset.*)/i,	
	release:false
});
fis.config.get('roadmap.path').unshift({
...
/****** rem 配置 ******/
fis.config.set('modules.postprocessor.css', ['px2rem']);
fis.config.set('settings.postprocessor.px2rem', 
{
