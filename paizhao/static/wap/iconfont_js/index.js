...
module.exports = function(ret, conf, settings, opt) {
    ...
    var pagePathRex = new RegExp((projectPath + settings.pagePath));
    ...
    _.map(files, function(subpath, file) {
        ...
        if (file.isHtmlLike || ext.ext === '.tpl') {
            ...
            /*console.log(ext.dirname)
            console.log(projectPath + settings.pagePath)
            console.log(pagePathRex)
            console.log(pagePathRex.test(ext.dirname))
            console.log("==========")*/

            // 需要添加css的页面
            if (ext.dirname === projectPath || pagePathRex.test(ext.dirname) ) {
                pages.push(file);
            }
