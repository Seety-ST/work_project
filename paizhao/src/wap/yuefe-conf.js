...
var domain_info = 
{
    ...
    "hst" : 
    { 
        "dev" : "http://localhost/yue_supe/loc"
    }
};
...
fis.media('predev').match('*', {
    charset: 'gbk',
    deploy: [
                fis.plugin('local-deliver', deploy_info.dev)
            ]
});
...
.match(/\/style\/ui\/(paizhao-ui-*.*)|(reset.*)/i,{
    release : false
})
.match('modules/**.css',{
...
// tag style in vue file
fis.match('yue-ui/**.vue:css', {
  rExt: 'css',
  parser: [
  fis.plugin('node-sass', {})
    ],
...
if(mode.trim() =='is_pub')
{
    ...
    .match('/style/common/main.scss', 
    {
        packTo: 'style/'+todate+'/main.css',
        domain:domain  
    })
    .match('/modules/global/**.js', {        
...
fis.set('modules.prepackager',function(ret,content,conf,settings)
{   
