define('wap-kids:upload/index', function(require, exports, module) {
  ...
  yue_topic_upload_class.prototype = {
      ...
      init_wx: function init_wx(params) {
          var self = this;
          ...
          if (self.type == 'wx') {
              ...
              var default_params = {
                  ...
                  platform: '',
                  choose_success: function choose_success(res) {
                      //选图完成时调用该方法
                  ...
                  choose_cancel: function choose_cancel(res) {
                      //取消选图时调用该方法
                  },
                  choose_fail: function choose_fail(res) {
                      //选图失败时调用该方法
                  },
                  upload_trigger_str: "uploadImages_test", //上传图片url事件触发自定义字符串 默认值uploadImages
                  upload_success: function upload_success(resa, index, total_index) {
                      //成功上传完每张图都会调用该方法 index为上传序号 0开始
  
                  },
                  upload_cancel: function upload_cancel(resa, index, total_index) {
                      //取消上传完每张图都会调用该方法 index为上传序号 0开始
                  },
                  upload_fail: function upload_fail(resa, index, total_index) {
                      //上传失败完每张图都会调用该方法 index为上传序号 0开始
  
                  },
                  upload_success_all: function upload_success_all(media_obj_list, index, total_index) {
                  ...
                  get_imgUrl_beforeSend: function get_imgUrl_beforeSend() {
                      //发送图片路径ajax前调用
  
                  },
                  get_imgUrl_success: function get_imgUrl_success(data) {
  
                      //发送图片路径ajax成功后调用
                      console.info(data);
                  },
                  get_imgUrl_error: function get_imgUrl_error(err) {
                      //发送图片路径ajax错误时调用
  
                  },
                  get_imgUrl_complete: function get_imgUrl_complete() {
                      //发送图片路径ajax完成后调用
      ...
      init_app: function init_app(params) {
          var self = this;
      ...
      init_web: function init_web(params) {
          var self = this;
          ...
          if (self.type == 'web') {
              ...
              var default_params = (_default_params = {
                  auto: true,
                  file_num_limit: 1,
                  ...
                  fileNumLimit: 20
              }, _defineProperty(_default_params, 'fileSizeLimit', 200 * 1024 * 1024), _defineProperty(_default_params, 'fileSingleSizeLimit', 50 * 1024 * 1024), _default_params);
      ...
      action: function action() {
          ...
          switch (self.type) {
              ...
              case "app":
                  ...
                  // 初始化上传组件
                  App.isPaiApp && App.upload_img(m_type, self.app_params, function (data) {
                      console.log(data);
